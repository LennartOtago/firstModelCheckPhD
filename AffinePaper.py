...
ax1.plot(nonLinAx, tang_heights_lin, color = 'k',marker = 'o', linestyle = '', markersize=3,  label = 'noise-free data', zorder = 1)
ax1.plot(y, tang_heights_lin, color = 'r', marker = 'o',  linestyle = '',linewidth = 0.5, markersize=7, label = 'noisy data', zorder = 0)
ax1.set_xscale('log')
ax1.legend()
...
ax4.plot(linTestDat,tang_heights_lin, linestyle = '', marker = '*', label = r'linear $\bm{A}_L\bm{x}$', markersize = 18 , zorder = 0, color = DatCol )
relErr = np.linalg.norm( MapLinTestDat -  nonLinTestDat) / np.linalg.norm(MapLinTestDat) * 100
ax4.plot(MapLinTestDat,tang_heights_lin, linestyle = '', marker = '*', label = r'$\bm{MA}_L\bm{x}$' + f', rel.~Err.: {relErr:.2f} \%', markersize = 7, zorder = 2, color ='k')
ax4.plot(nonLinTestDat,tang_heights_lin, linestyle = '', marker = 'o', label = r'non-linear $\bm{A}(\bm{x})$', markersize = 10, zorder = 1, color = 'r')
ax4.legend()
