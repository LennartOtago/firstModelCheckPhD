...
def curvature(lambd, sig, beta, xi):
    ...
    if isinstance(lambd, float):
        f  = np.divide((sig ** 2), (sig ** 2 + lambd ** 2)) # ok
        cf = 1 - f # ok
...
def l_cuve(u, sig, bm, plotit = False):
    ...
    Adapted from Per Christian Hansen, DTU Compute, October 27, 2010.
    Inputs:
        u: left side matrix computed from svd (size: Nm x Nu(changed by Lennart from Nm x Nm)) - Nm is the number of measurement points
